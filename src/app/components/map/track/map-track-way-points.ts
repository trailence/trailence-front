...
export class MapTrackWayPoints {
  ...
  private load(): void {
    ...
    if (this.track instanceof Track) {
      this.subscription = combineLatest([this.track.computed.wayPoints$, this.track.computed.mapService.getPoiIcon$('guidepost')]).subscribe(([list, guidepostIcon]) => this.loadFromTrack(list, guidepostIcon));
    } else {
