...
import { filter, first, map, Observable, of, switchMap, zip } from 'rxjs';
...
export class FindDuplicatesComponent implements OnInit, OnDestroy {
  ...
  what = 'inside';
  withCollection?: TrailCollection;
  threshold = 85;
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(this.collectionService.getAll$().pipe(collection$items()).subscribe(list => this.otherCollections = list.filter(c => c.uuid !== this.collectionUuid)));
  }
  ...
  start(): void {
    ...
    if (this.what === 'inside') {
      ...
      toCompare$ = this.trailService.getAll$().pipe(
        collection$items(),
        first(),
        map(list => ({source: list.filter(t => t.collectionUuid === this.collectionUuid), target: list.filter(t => t.collectionUuid === this.withCollection!.uuid)})),
        ...
        map(([source, target]) => {
          ...
          for (let i = 0; i < source.length; ++i) {
            for (let j = 0; j < target.length; ++j) {
              toCompare.push({source: source[i], target: target[j]});
            }
          }
          return toCompare;
        })
    ...
    toCompare$.pipe(first()).subscribe(toCompare => this.next(toCompare, 0, Date.now()));
  }

  private next(toCompare: {source: {trail: Trail, track: Track}, target: {trail: Trail, track: Track}}[], index: number, startTime: number): void {
    ...
    if (similarity * 100 < this.threshold) {
      if (Date.now() - startTime < 1000) this.next(toCompare, index + 1, startTime);
      else setTimeout(() => this.next(toCompare, index + 1, Date.now()), 0);
      return;
    }
    ...
    this.launchNext = () => {
      this.processing = true;
      this.launchNext = undefined;
      setTimeout(() => this.next(toCompare, index + 1, Date.now()), 0);
    };
  }

  deleteTrail(trail: Trail): void {
    this.trailMenuService.confirmDelete([trail], false);
  }
