...
describe('Test Trail and Track DTOs', () => {
  ...
  it('gpx-001', async () => {
    ...
    expect(imported[0].tracks).toHaveSize(1);
    const trailDto = imported[0].trail.toDto();
    const trackDto = imported[0].tracks[0].toDto();
    ...
    expect(trail.name).toBe(imported[0].trail.name);
    expect(trail.description).toBe(imported[0].trail.description);
    expect(Math.floor(track.metadata.distance)).withContext('distance').toBe(Math.floor(imported[0].tracks[0].metadata.distance));
    expect(track.metadata.positiveElevation).withContext('positiveElevation').toBe(imported[0].tracks[0].metadata.positiveElevation);
    expect(track.metadata.negativeElevation).withContext('negativeElevation').toBe(imported[0].tracks[0].metadata.negativeElevation);
    expect(track.metadata.highestAltitude).withContext('highestAltitude').toBe(imported[0].tracks[0].metadata.highestAltitude);
    expect(track.metadata.lowestAltitude).withContext('lowestAltitude').toBe(imported[0].tracks[0].metadata.lowestAltitude);
    expect(track.metadata.duration).withContext('duration').toBe(imported[0].tracks[0].metadata.duration);
  });
