...
export class AdminStatsBuilder implements GraphProvider<AdminStatsConfig> {
  ...
  buildGraphConfig(source: AdminStatsConfig, width: number, height: number, styles: CSSStyleDeclaration): Observable<GraphConfig | undefined> {
    ...
    const secondaryColor = styles.getPropertyValue('--ion-color-secondary');
    const tertiaryColor = styles.getPropertyValue('--ion-color-tertiary');
    const xLabel = this.getXLabelProvider(source);
    const requests: Observable<StatsValueDto[]>[] = [];
    requests.push(this.injector.get(HttpService).get<StatsValueDto[]>(environment.apiBaseUrl + '/admin/stats/v1?type=' + source.type + '&aggregation=' + source.aggregation));
    if (source.type.startsWith('nbActiveUsers'))
      requests.push(this.injector.get(HttpService).get<StatsValueDto[]>(environment.apiBaseUrl + '/admin/stats/v1?type=nbInactiveUsers' + source.type.substring(13) + '&aggregation=' + source.aggregation));
    ...
    .pipe(
      map(stats => ({
        ...
        data: {
          datasets: stats.map((values, index) => ({
            data: this.buildData(source.aggregation, values),
            borderColor: index === 0 ? tertiaryColor : secondaryColor,
          })),
        },
