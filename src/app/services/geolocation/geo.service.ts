...
export class GeoService {
  ...
  private findSingleWayStartingAt(lat: number, lng: number, ways: Way[], i: number, permission: WayPermission | undefined): number {
    let found = -1;
    for (; i < ways.length; ++i) {
      const way = ways[i];
  ...
  private merge(w1: Way, w2: Way): void {
