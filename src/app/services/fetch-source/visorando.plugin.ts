...
export class VisorandoPlugin implements FetchSourcePlugin {
  ...
  public owner = 'visorando';

  private sanitizer: DomSanitizer;

  private tableInfos: Table<TrailInfoDto, string>;
  private tableTrails: Table<TrailDto, string>;
  private tableFullTracks: Table<TrackDto, string>;
  private tableSimplifiedTracks: Table<SimplifiedTrackDto, string>;
  private tableMetadata: Table<TrackMetadataSnapshot, string>;
  ...
  public fetchTrailInfo(url: string): Promise<TrailInfo | null> {
    ...
    return fromDb.then(info => info?.info ?? window.fetch(url, {mode: 'cors'}).then(response => response.text()).then(text => {
      ...
      for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
        ...
        if (article) {
          ...
          for (let pIndex = 0; pIndex < paragraphs.length; pIndex++) {
            let content = this.sanitize(paragraphs.at(pIndex)!.textContent!);
      ...
      for (let i = 0; i < buttons.length; ++i) {
        ...
        } else if (data && data.indexOf('task=pdfRando')) {
          const j = data.lastIndexOf('=');
  ...
  public searchByArea(bounds: L.LatLngBounds): Promise<Trail[]> {
    ...
    .then(items => {
      const nextItems = (found: Trail[], items: {id: number, url: string}[], startIndex: number): Promise<Trail[]> => {
        if (found.length >= 50 || startIndex >= items.length) return Promise.resolve(found);
        const toFetch = items.slice(startIndex, Math.min(items.length, startIndex + 50 - found.length));
        ...
        return Promise.all(trails$).then(trails => trails.filter(trail => !!trail).map(dto => new Trail(dto)))
  ...
  private fetchTrailByGpx(idTrail: string, idGpx: string, info: TrailInfo) {
    return this.tableTrails.get(idTrail).then(t => t ? t : this.fetchGpx(idTrail, idGpx, info));
  }
  ...
  private fetchGpx(idTrail: string, idGpx: string, info: TrailInfo) {
    ...
    .then(gpx => {
      ...
      if (info.wayPoints && info.wayPoints.length > 0) {
        for (let i = 0; i < info.wayPoints.length && i < gpx.tracks[0].wayPoints.length; ++i) {
          if (info.wayPoints[i].description && gpx.tracks[0].wayPoints[i].description.length === 0)
            gpx.tracks[0].wayPoints[i].description = info.wayPoints[i].description!;
        }
      }
