...
import { catchError, combineLatest, firstValueFrom, from, map, Observable, of, switchMap } from 'rxjs';
import { SearchResult } from './fetch-source.interfaces';
import { HttpService } from '../http/http.service';
import { environment } from 'src/environments/environment';
import { Trail } from 'src/app/model/trail';
import { TrailDto } from 'src/app/model/dto/trail';
import { SimplifiedPoint, TrackMetadataSnapshot } from '../database/track-database';
...
export class TrailencePlugin extends PluginWithDb<TrailInfoDto> {
  ...
  constructor(
    ...
    super(injector, 'trailence_public', 'uuid', 'uuid');
  }
  ...
  public override getFullTrack(uuid: string): Promise<Track | null> {
    return super.getFullTrack(uuid)
    .then(t => {
      if (t) return t;
