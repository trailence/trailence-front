...
describe('Preferences', () => {
  ...
  it('Login, import a GPX', async () => {
    ...
    await checkTrail(trailsList, expectedDateEn, 'Duration', '3h12 (≈ 2h35)', 'Distance', '5.335 mi', 'Ascent', '+ 1,053 ft', 'Descent', '- 971 ft');
  });
  ...
  it('Change language to french', async () => {
    ...
    await checkTrail(await collection.trailsAndMap.openTrailsList(), expectedDateFr, 'Durée', '3h12 (≈ 2h35)', 'Distance', '8,59 km', 'Dénivelé positif', '+ 321 m', 'Dénivelé négatif', '- 296 m');
  });
  ...
  it('Change back to english', async () => {
    ...
    await checkTrail(await collection.trailsAndMap.openTrailsList(), expectedDateEn, 'Duration', '3h12 (≈ 2h35)', 'Distance', '5.335 mi', 'Ascent', '+ 1,053 ft', 'Descent', '- 971 ft');
  });
  ...
  it('Reset all, delete trail, and synchronize', async () => {
    ...
    await TestUtils.retry(
      async () => await checkTrail(trailsList, expectedDateEn, 'Duration', '3h12 (≈ 2h35)', 'Distance', '5.335 mi', 'Ascent', '+ 1,053 ft', 'Descent', '- 971 ft'),
      20, 1000
    );
