...
export class TrailenceClient {
  ...
  private async adminToken() {
    if (!this.adminAccessToken)
      this.adminAccessToken = await this.login(this.adminUsername, this.adminPassword);
    return this.adminAccessToken;
  }

  private async userToken() {
    if (!this.remoteAccessToken)
      this.remoteAccessToken = await this.login(this.remoteUsername, this.remotePassword);
