...
import { PointDescriptor } from '@trailence/model/point-descriptor.js';
import { TrailDto } from '@trailence/model/dto/trail.js';
...
export class GeoTrekImport extends Importer {
  ...
  private async toTrailenceDtos(trek: GeoTrekDto): Promise<{trail: TrailDto, track: PointDescriptor[][], wayPoints: WayPoint[], photos: PhotoInfo[]}> {
    ...
    // waypoints
    const wayPointModule = await import('@trailence/model/way-point.js');
    ...
    const pointDescriptorModule = await import('@trailence/model/point-descriptor.js');
  ...
  private async downloadPhotos(owner: string, trailUuid: string, infos: PhotoInfo[]): Promise<{blob: Blob, photo: Photo}[]> {
    ...
    for (const info of infos) {
      try {
        ...
        const module = await import('@trailence/worker/functions/import-photo.js');
        photos.push(await (
