...
function readI18nDir() {
  ...
  for (const l of languages.values()) {
    ...
    if (!l.files['png']) throw 'Missing png file for language ' + l.language;
  }
...
function checkKeys(object1, object2, lang1, lang2, path) {
  for (const k of Object.keys(object1)) {
    const v = object1[k];
    if (!object2[k]) throw 'Key ' + path + '/' + k + ' present in ' + lang1 + ' is missing in ' + lang2;
    const v2 = object2[k];
    if (typeof v !== typeof v2) throw 'Key ' + path + '/' + k + ' in lang1 is a ' + (typeof v) + ' but is a ' + (typeof v2) + ' in ' + lang2;
    if (typeof v === 'string') continue;
    checkKeys(v, v2, lang1, lang2, path + '/' + k);
  }
}
